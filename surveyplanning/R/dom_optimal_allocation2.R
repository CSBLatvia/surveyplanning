dom_optimal_allocation2 <- function(id, Dom, H, Y, Rh=NULL,
  ...
  a1 <- copy(aa)
  a1[, cv:=10*get(sup_cv1)]
  a1[, nh:=1]

  while (nrow(a1[cv>sup_cv])>0){
         a1[cv > sup_cv, sum_apj:=as.integer(sum_apj)+1]
         a1[, (c("nh", "cv")):=NULL]

         t1 <- a1[poph_sample!=0, optsize(H=strata1, n=sum_apj[1], poph="poph", 
                                          s2h = "s2_Y", Rh = "Rh", deffh="deffh",
                                          dataset = .SD), by=c(dom1)]
         t1 <- t1[, c(dom1, strata1, "nh"), with = FALSE][, nh:=round(nh)]
         a2 <- merge(a1, t1, all.x = TRUE, by=c(dom1, strata1))
         a2[is.na(nh), nh:=0]
         a2[, nh:=nh + sample100]
         ...
         as <- expvar(Yh="sum_Y", Zh=NULL, H=strata1, 
                      ...
                      Rh = "Rh", deffh = "deffh", Dom = dom1,
                      dataset = a2)$resultDom

         a1 <- merge(a2, as[,c(dom1, "cv"), with=FALSE], keyby=dom1)
      }             
